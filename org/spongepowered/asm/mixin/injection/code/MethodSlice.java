...
public final class MethodSlice {
   ...
   public ReadOnlyInsnList getSlice(MethodNode var1) {
      ...
      int var3 = this.find(var1, this.from, 0, 0, this.name + "(from)");
      int var4 = this.find(var1, this.to, var2, var3, this.name + "(to)");
      if (var3 > var4) {
   ...
   private int find(MethodNode var1, InjectionPoint var2, int var3, int var4, String var5) {
      if (var2 == null) {
         return var3;
      } else {
         LinkedList var6 = new LinkedList();
         ReadOnlyInsnList var7 = new ReadOnlyInsnList(var1.instructions);
         boolean var8 = var2.find(var1.desc, var7, var6);
         InjectionPoint.Selector var9 = var2.getSelector();
         if (var6.size() != 1 && var9 == InjectionPoint.Selector.ONE) {
            throw new InvalidSliceException(this.owner, String.format("%s requires 1 result but found %d", this.describe(var5), var6.size()));
         } else if (!var8) {
            if (this.owner.getContext().getOption(MixinEnvironment.Option.DEBUG_VERBOSE)) {
               logger.warn("{} did not match any instructions", new Object[]{this.describe(var5)});
            }

            return var4;
         } else {
            return var1.instructions.indexOf(var9 == InjectionPoint.Selector.FIRST ? (AbstractInsnNode)var6.getFirst() : (AbstractInsnNode)var6.getLast());
         }
      }
   }
